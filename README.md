...
- **探索** — ECHONET Lite 機器をマルチキャスト / ユニキャスト / サブネットスキャンで発見
- **状態取得** — 運転状態、設定温度、室温、湿度、消費電力などを EPC 経由で読み取り
- **常時監視** — 複数台のエアコンを定周期ポーリングし、JSON API / Web UI で表示
- **Prometheus エクスポーター** — `/metrics` でメトリクス公開 → Grafana でダッシュボード可視化
